...
class Net(nn.Module):
	...
	def forward(self, batch):
		...
		if 'loss' in self.output_type:
			output['label_loss'] = F.cross_entropy(logit, batch['label'],label_smoothing=label_smoothing)#0.5
...
def run_check_net():
	...
	xyz = [
		np.random.uniform(-1,1,(length[b],point_dim)) for b in range(batch_size)
	]
	xyz_mask = [
		np.random.uniform(0,1,(length[b])) for b in range(batch_size)
	]
	#---
	batch = {
		'label' : torch.from_numpy( np.random.choice(250,(batch_size))).long(),#.cuda(),
		'xyz' : [torch.from_numpy(x).float() for x in xyz],
		'xyz_mask' : [torch.from_numpy(x).float() for x in xyz_mask],
	}
