...
class ASLDataset(Dataset):
    def __init__(self, mode, train_path, pr_train_path, json_path, num_fold, 
                 fold_type, val_fold, max_length, transform=None):
        self.mode = mode
    ...
    def get_meta_df(self):
        ...
        if self.fold_type == 'random':
            df = read_kaggle_csv_by_part(num_fold=self.num_fold, TRAIN_FILE=self.train_file, SIGN_TO_IDX=self.sign_to_idx)
